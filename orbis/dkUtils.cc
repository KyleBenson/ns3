...
std::ostream& operator<<(std::ostream& out, const NKMap& m) {
    for (NKMap::const_iterator i = m.begin();
	 i != m.end();
	 ++i)
    {
...
std::ostream& operator<<(std::ostream& out, const NKKMap& m)
 {
    for (NKKMap::const_iterator i = m.begin();
	 i != m.end();
	 ++i)
    {
	for (NKMap::const_iterator j = i->second.begin();
	     j != i->second.end();
	     ++j)
	{
...
std::ostream& operator<<(std::ostream& out, const PKMap& m) {
    for (PKMap::const_iterator i = m.begin();
	 i != m.end();
	 ++i)
    {
	out << i->first << " " << i->second << std::endl;
...
std::ostream& operator<<(std::ostream& out, const PKKMap& m)
 {
    for (PKKMap::const_iterator i = m.begin();
	 i != m.end();
	 ++i)
    {
	for (PKMap::const_iterator j = i->second.begin();
	     j != i->second.end();
	     ++j)
	{
	    out << i->first << " " << j->first << " " << j->second 
...
bool read1kDistribution(const std::string& fileName,
		       NKMap& degNumNodesMap)
{
	...
	if (scanfResult == EOF) 
	    break;
	if (scanfResult != 2) {
	    std::cerr << "Error(2) on line: " << inLine 
	...
	{
	    std::cerr << "Repeated degree: " << degree 
		      << " line # " << lineNumber << std::endl;
	    return false;
	}
...
void NKKToPKK(const NKKMap& nkkmap, PKKMap& pkkmap)
{
	     ...
	     ++nkIter)
	{
		...
		pkkmap[degree1][degree2] = float(edges) / ((float)numEdges * 2);
	}
    }
}
