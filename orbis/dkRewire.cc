...
int main(int argc, char *argv[])
{
    ...
    // getopt testing
    int param;
    while ((param = getopt_long(argc, argv, "i:o:k:w:t:r:a:h", 
				options, NULL)) != EOF)
    {
	switch((char)param)
	{
    ...
    cout << "Original graph had " << numCC << " connected components" 
	 << endl;
	...
	if (paramMap.find('a') != paramMap.end())
	{
	    ...
	    rewiredEdges = targetRandomizeGraph1k(g, 
						  rewireLimit, 
						  timeLimit, 
						  randomSeed, 
						  r,
	...
	else
	{
	    ...
	    rewiredEdges = randomizeGraph1k(g, 
					    rewireLimit, 
					    timeLimit, 
					    randomSeed, 
					    true);
	...
	break;
    case(2):
	rewiredEdges = randomizeGraph2k(g, 
					rewireLimit, 
					timeLimit, 
					randomSeed, 
					true);
	break;
    case(3):
	rewiredEdges = randomizeGraph3k(g, 
					rewireLimit, 
					timeLimit, 
					randomSeed, 
					true);
