...
void 
dkTopoGen2k_connectStubs(VertexListGraph& g,
			 std::list<int>& randomStubIds,
			 DegreeStubListMap &degreeStubListMap,
			 ...
			 NKKMap& workingMap)
{
	...
	for (i = targetList.begin(); i != targetList.end(); ++i)
	{
	    ...
	    if ((adjacencyMap.find(NodeIdPair(stub.nodeid, 
					      potentialStub.nodeid))
		 != adjacencyMap.end()) ||
		(adjacencyMap.find(NodeIdPair(potentialStub.nodeid, stub.nodeid))
		 != adjacencyMap.end()))
	    {
...
template <class VertexListGraph>
int
dkTopoGen2k(VertexListGraph& g, const NKKMap &nkkMap)
{
    typedef boost::graph_traits<VertexListGraph> GraphTraits;
    typedef typename GraphTraits::edge_iterator edge_iterator;
    typedef typename GraphTraits::edge_descriptor edge_descriptor;
    ...
    NKMap nkMap;
    for (NKKMap::const_iterator i = nkkMap.begin();
	 i != nkkMap.end(); ++i) 
    {
	int degree1 = i->first;
	int tmpdd = 0;
