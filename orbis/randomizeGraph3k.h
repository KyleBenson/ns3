...
namespace randomGraph3k {
    ...
    inline void
    revertToPreReWired(typename graph_traits<VertexListGraph>::vertex_descriptor v1,
    ...
    // helper function!
    template <class VertexListGraph>
    inline int swapEdge3k(unsigned int ind1, // index into edgeList
	...
	{
	    // #Compre Z with RfZ
	    for (typename NodeDegreeDistributionMap::iterator i = nodeDegreeDistribution.begin();
		 i != nodeDegreeDistribution.end();
		 ++i)
	    {
		int key = i->first;
...
template <class VertexListGraph>
int randomizeGraph3k(VertexListGraph& G, 
		      ...
		      bool verbose = false) 
{
	...
	if (nodeDegreeDistribution.find(deg) ==
	    nodeDegreeDistribution.end())
	{
	...
	for (unsigned int ind2 = ind1; ind2 < numEdges; ind2++)
	{
	    ...
	    if ((k3 == 1) || (k4 == 1))
		continue;
	    result = randomGraph3k::swapEdge3k(ind1, ind2, edgeList, 
					       maxDegree, 
