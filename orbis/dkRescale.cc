...
void dkRescale2k(const NKKMap& originalMap, NKKMap& newMap, 
...
pair<int,int> dkScaleMediumDegrees(const PKMap& pkmap, 
				   const NKMap& nkmap, 
				   NKMap& rescaledMap,
				   list<int>& degreeList,
				   int numNodes, int numNodes_new,
...
int main(int argc, char* argv[])
{
	...
	("dk,k", po::value<int>(&opt_k), "{1, 2}-dk distribution")
	("input,i",po::value<string>(&opt_infile), "input distribution file")
	("nodes,n",po::value<int>(&opt_newNumNodes), "number of nodes in target graph")
	;

    po::options_description desc_optional("Optional arguments");
    desc_optional.add_options()
	("help,h", "produce help message")
	("prob,p", "print probability distributions instead of degree distributions")
	("max_degree,m", po::value<int>(&opt_maxDegree), "Maximum degree (330 recommended for router topos")
	("seed,s", po::value<int>(&opt_randomSeed), "Random seed (default: system time)")
	;
	...
	if (!read2kDistribution(opt_infile, nkkmap) ||
	    (nkkmap.empty()))
	{
	...
	if (!read1kDistribution(opt_infile, nkMap) ||
	    (nkMap.empty()))
	{
...
void dkCalcRescaleUpDistribution(const PKMap& rescaledPKMap,
				 const ScaleMapping& newToOldMap,
				 const NKKMap& nkkOrig,
				 const NKMap& nkOrig,
				 NKKMap& nkkRescaled,
				 int k_knee1, int k_knee2,
				 int X, int X_prime,
				 int numNodes,
				 int numNodes_new)
{
	...
	for (NKMap::const_iterator j = nkOrig.begin();
	     j != nkOrig.end();
	     ++j)
	{
	    int k = j->first;
	     ...
	     ++j)
	{
	    ...
	    if ((frac > 1.0) && (degree2_orig <= k_knee2) && (degree2_orig >= k_knee1))
	    {

		nkkRescaled_tmp[degree1][degree2] = int ( ((deg_deg_percent[degree2_orig] * 
							degree1 * numNodes_new * 
							rescaledPKMap.find(degree1)->second ) ) / frac + 0.5 );
	    } 
	    else 
	    {
		float totfrac = 0;
		for (list<int>::const_iterator tmpDeg = newToOldMap.find(degree2)->second.begin();
		...
		nkkRescaled_tmp[degree1][degree2] = int((totfrac * degree1 * numNodes_new *
						     rescaledPKMap.find(degree1)->second + 
						     .5));
	    }
	}
    }
    ...
    for (NKKMap::iterator i = nkkRescaled_tmp.begin();
	 i != nkkRescaled_tmp.end();
	 ++i)
    {
	int k1 = i->first;

	for (NKMap::iterator j = i->second.begin();
	     j != i->second.end();
	     ++j)
	{
	    int k2 = j->first;
...
void dkRescale2k(const NKKMap& originalMap, 
		 NKKMap& rescaledMap, 
		 int numNodes_new,
		 int maxDegree)
{
//    PKKMap pkkRescaledMap;  // Rescaled probability distribution
    ...
    pair<int,int> supp_M;
    supp_M = dkRescale1k(nkmap, nkRescaledMap, numNodes_new, newToOldMap, maxDegree);
    ...
    dkCalcRescaleUpDistribution(rescaledPKMap,
				newToOldMap,
				originalMap,
				nkmap,
				rescaledMap,
...
pair<int,int> dkRescale1k(const NKMap& originalMap, 
			  NKMap& rescaledMap, 
			  int numNodes_new,
			  ScaleMapping& newToOldMap,
			  int maxDegree)
{
	...
	cerr << "Warning - scale factor of greater than 10, weird behaviors may ensue" << endl;
    ...
    // components
    for (NKMap::const_iterator i = originalMap.begin();
	 i != originalMap.end();
	 ++i)
    {
	int degree = i->first;
	...
	dkScaleHighDegrees(pkmap, rescaledMap, 
				highDegreeList,  
				numNodes, numNodes_new, 
				k_knee1, maxDegree,
				newToOldMap);
	...
	dkScaleSmallDegrees(pkmap, rescaledMap, 
			    smallDegreeList, numNodes, numNodes_new,
			    newToOldMap);
	...
	supp_M = dkScaleMediumDegrees(pkmap, originalMap, rescaledMap, 
				      mediumDegreeList,  numNodes, numNodes_new,
				      newToOldMap);
			 ...
			 ScaleMapping& newToOldMap)
{
	...
	rescaledMap[degree] = (int)round(pkmap.find(degree)->second * 
					(float)numNodes_new);
				   ...
				   ScaleMapping& newToOldMap)
{
    ...
    kh_nodes = (int) round(pkmap.find(degreeList.back())->second * 
			   float(numNodes));
    ...
    u_prime = 0;
    int tmp = 0;
    for (NKMap::const_iterator i = rescaledMap.begin();
	 i != rescaledMap.end();
	 ++i)
    {
    ...
    for (NKMap::const_iterator i = nkmap.begin();
	 i != nkmap.end();
	 ++i)
    {
	int deg = i->first;
	...
	float slope = (startPoint.y - endPoint.y) /  
    ...
    for(list<int>::iterator i = degreeList.begin();
	i != degreeList.end();
	++i)
    {
...
// 	 << degreeList.back() << " and:" << degreeList.front() << endl;
	...
	int newDegree = (int) ceil(degree * 
				   double(numNodes_new) / numNodes);
	...
	if (maxDegree && (newDegree > maxDegree))
	{
	    ...
	    rescaledMap[maxDegree] = (int) round(float(numNodes) * 
						 (pkmap.find(degree)->second));
...
// 	    cerr <<  "WAWOOHA: " << rescaledMap[maxDegree] << endl;
	...
	// Add degree
	rescaledMap[newDegree] = (int) round(float(numNodes) * 
					     (pkmap.find(degree)->second));
				      ...
				      float xmean, float ymean)
{
    ...
    float denom_y = 0.0;
    int cnt = 0; 
    for (list<Point>::const_iterator i = points.begin();
	 i != points.end();
	 ++i)
    {
	const Point& p = *i;
...
int dkGetKnee1(const NKMap& nkmap, const PKMap& pkmap, int numNodes)
{
	...
	float r = calculateCorrelationCoefficient(points, 
						  movingAverage_x.getAverage(),
						  movingAverage_y.getAverage());
