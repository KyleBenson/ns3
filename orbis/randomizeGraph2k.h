...
namespace randomGraph2k {
    ...
    inline int swapEdge2k(typename graph_traits<VertexListGraph>::vertex_descriptor v1,
...
template <class VertexListGraph>
int randomRewireVertex2k(VertexListGraph& G, 
			 ...
			 std::list<typename boost::graph_traits<VertexListGraph>::edge_descriptor>& candidateEdgeList)
{
    ...
    for (tie(edgeIter, edgeIterEnd) = out_edges(v, G);
...
template <class VertexListGraph>
int randomRewireEdge2k(VertexListGraph& G, 
		       ...
		       std::list<typename boost::graph_traits<VertexListGraph>:: edge_descriptor>& edgeList)
{
	...
	for(tie(edgeIter, edgeIterEnd) = edges(G);
	    edgeIter != edgeIterEnd;
	    ++edgeIter)
	{
	    if (e != *edgeIter)
...
template <class VertexListGraph>
int randomizeGraph2k(VertexListGraph& G, 
		     int rewireSuccessLimit, 
		     ...
		     bool verbose=false)
{
	...
	// (i.e. non-0)
	if (rewireAttemptLimit && 
	    (rewireAttemptCount > rewireAttemptLimit))
	{
	    break;
	}
	if (rewireSuccessLimit && 
	    (rewireSuccessCount > rewireSuccessLimit))
	{
	    break;
	}
	...
	if (timeLimit_sec)
	{
 	...
 	assert(ind1 < edgeList.size()); // should never happen
 	int result = 0;
 	int tmp_cnt = 1;
	...
	while ((result == 0) && (tmp_cnt < 20))
	{
	    ...
	    assert(ind2 < edgeList.size());// should never happen
	    ...
	    result = randomGraph2k::swapEdge2k(edgeList[ind1].first, 
					       edgeList[ind1].second,
					       edgeList[ind2].first,
					       edgeList[ind2].second,
					       G);
